...
/**
 * Remove order and customer data that came from the store. Called when a
 * store is disconnected or uninstalled, while it is the business's only
 * store, so every synced order belongs to it. Products are kept until
 * shop/redact (or reconnect) so the merchant's catalog isn't lost to an
 * accidental uninstall; they're hidden from the AI once disconnected.
 */
export async function purgeStoreOrderData(businessId: string) {
  const admin = createAdminClient();
...
async function appUninstalled(connectionId: string, businessId: string, shop: string): Promise<"processed" | "ignored"> {
  ...
  await purgeStoreOrderData(businessId);
...
/** 48 hours after uninstall: delete everything that came from the store. */
async function shopRedact(businessId: string, shop: string): Promise<"processed" | "ignored"> {
  ...
  const ids = (conns ?? []).filter((c) => c.shop_domain === shop).map((c) => c.id);
  if (ids.length) {
    await must(admin.from("products").delete().eq("business_id", businessId).in("shopify_connection_id", ids));
    await must(admin.from("shopify_credentials").delete().in("connection_id", ids));
  }
  // Orders don't record their store; they're only safe to clear when no other store is connected.
  if (!live.length) await purgeStoreOrderData(businessId);
