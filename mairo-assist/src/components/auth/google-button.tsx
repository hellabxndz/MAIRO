...
/** "Continue with Google" — used on both sign-in and sign-up. */
export function GoogleButton({ next }: { next?: string }) {
  return (
    <form action={signInWithGoogleAction}>
      <input type="hidden" name="next" value={next ?? ""} />
      <SubmitButton variant="secondary" size="lg" className="w-full [&_svg]:size-5" pendingText="Opening Google…">
        <GoogleMark />
...
export function OrDivider() {
  return (
    <div className="my-5 flex items-center gap-3 text-xs text-fg-subtle" role="separator">
