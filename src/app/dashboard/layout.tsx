...
import { recentNotifications, unreadCount } from "@/lib/notifications/notify";
import { toBellItem } from "@/lib/notifications/present";
import { NotificationBell } from "@/components/mairo/notification-bell";
...
export default async function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  // The bell's contents, rendered server-side on each navigation. These are
  // generated by a daily sweep, so that is exactly as fresh as the data is —
  // polling would cost a request a minute to say the same thing.
  const [bellRows, unread] = await Promise.all([
    recentNotifications(organizationId, 6),
    unreadCount(organizationId),
  ]);
